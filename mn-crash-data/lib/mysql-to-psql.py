...
# Copied from https://github.com/lanyrd/mysql-postgresql-converter
"""
...
def parse(input_filename, output_filename):
    ...
    for i, line in enumerate(input_fh):
        ...
        logging.write(
            "\rLine %i (of %s: %.2f%%) [%s tables] [%s inserts] [ETA: %i min %i sec]"
            % (
                i + 1,
                num_lines,
        ...
        line = line.decode("utf8").strip().replace(
            r"\\", "WUBWUBREALSLASHWUB").replace(r"\'", "''").replace(
                "WUBWUBREALSLASHWUB", r"\\")
        # Ignore comment lines
        if line.startswith("--") or line.startswith("/*") or line.startswith(
                "LOCK TABLES") or line.startswith(
                    "DROP TABLE") or line.startswith(
                        "UNLOCK TABLES") or not line:
            continue
        ...
        if current_table is None:
            ...
            # Inserting data into a table?
                ...
                output.write(
                    line.encode("utf8").replace("'0000-00-00 00:00:00'",
                                                "NULL") + "\n")
                num_inserts += 1
            # ???
            else:
        ...
        # Inside-create-statement handling
            ...
            if line.startswith('"'):
                ...
                try:
                    ...
                    extra = ""
                extra = re.sub("CHARACTER SET [\w\d]+\s*", "",
                ...
                if final_type:
                    cast_lines.append(
                        ...
                        % (current_table, name, name, final_type, name,
                           final_type))
                # ID fields need sequences [if they are integers?]
                if name == "id" and set_sequence is True:
                    sequence_lines.append("CREATE SEQUENCE %s_id_seq" %
                                          (current_table))
            ...
            # Is it a constraint or something?
                ...
                foreign_key_lines.append(
                    "ALTER TABLE \"%s\" ADD CONSTRAINT %s DEFERRABLE INITIALLY DEFERRED"
                    % (current_table,
                       line.split("CONSTRAINT")[1].strip().rstrip(",")))
                foreign_key_lines.append(
                    "CREATE INDEX ON \"%s\" %s" %
                    (current_table, line.split("FOREIGN KEY")[1].split(
                        "REFERENCES")[0].strip().rstrip(",")))
            elif line.startswith("UNIQUE KEY"):
                creation_lines.append(
                    "UNIQUE (%s)" % line.split("(")[1].split(")")[0])
                ...
                fulltext_key_lines.append(
                    "CREATE INDEX ON %s USING gin(to_tsvector('english', %s))"
            ...
            # Is it the end of the table?
                ...
                for i, line in enumerate(creation_lines):
                    output.write("    %s%s\n" %
                                 (line, ","
                                  if i != (len(creation_lines) - 1) else ""))
                output.write(');\n\n')
                current_table = None
            # ???
            else:
